...
// let parseEnver =(fileContent)=> {
// 	return fileContent.toBuffer()
	//parse all the thing env file
	//expected format: \nPASSWORD=PaSSw0rd\nVARIABLE2=STRING2'
	//return parse contont without PORT
// }
...
// 		console.log( {'repo':repo , 'entryPoint': parsePackage});
// 	})
// });

inquirer.prompt(filepath).then(answers =>{
	let content;
	fs.readFile(answers.toString(), function read(err,data){
		if(err){
			throw err;
		}
			content=data;
			console.log('line 110', content)
	});
function processFile(){
	console.log('line 113', content);
}
	console.log('line 115', JSON.stringify(processFile));
});
